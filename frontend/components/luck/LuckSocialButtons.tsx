...
export function LuckSocialButtons() {
  const [msg, setMsg] = useState<string | null>(null);

  const simulate = (provider: string) => {
    setMsg(`Conectando com ${provider}...`);
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <button onClick={() => simulate('Google')} className="lk-press" style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
          padding: '12px', borderRadius: 10, border: '1.5px solid var(--gray-soft)', background: 'white',
          cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: 'var(--ink)',
        }}>
        ...
        <button onClick={() => simulate('Facebook')} className="lk-press" style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
          padding: '12px', borderRadius: 10, border: 'none', background: '#1877F2',
          cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: 'white',
        }}>
        ...
        {msg && (
          <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--navy)', fontWeight: 600, padding: '4px 0' }}>
            {msg}
          </div>
        )}
      </div>
